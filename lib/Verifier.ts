...
export class Verifier {
  ...
  private buildRP(definitionId: DefinitionIds): RP {
    return (
      RP.builder({
        requestVersion: SupportedVersion.JWT_VC_PRESENTATION_PROFILE_v1,
      })
        ...
        .withInternalSignature(
          process.env.RP_PRIVATE_KEY_HEX!,
          process.env.RP_DID!,
          process.env.RP_DID_KID!,
          SigningAlgo.ES256K
        )
        ...
        .withPresentationDefinition(
          { definition: getPresentationDefinition(definitionId) },
          PropertyTarget.REQUEST_OBJECT
        )
...
export const verificationCallback = async (
  args: any
