...
export function getPresentationDefinition(
  id: string | DefinitionIds
): IPresentationDefinition {
  // TODO: This should come from a database or your agent. We are just showing a simple example here
  if (
    id === DefinitionIds.ENTRA_VERIFIED_ID_AND_SPHEREON ||
    (typeof id === "string" &&
      (id.toLowerCase().includes("sphereon") ||
        id === DefinitionIds.ENTRA_VERIFIED_ID_AND_SPHEREON.valueOf()))
  ) {
    return entraAndSphereonCompatibleDefinition();
  } else if (
    id === DefinitionIds.ENTRA_VERIFIED_ID_ONLY ||
    id === DefinitionIds.ENTRA_VERIFIED_ID_ONLY.valueOf()
  ) {
